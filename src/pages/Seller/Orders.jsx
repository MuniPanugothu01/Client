...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get("/api/order/my-orders"); // ✅ use correct endpoint
      console.log("Fetched Orders:", data); // ✅ helpful for debugging
      if (data.success) {
        setOrders(data.orders);
      } else {
        toast.error(data.message);
                ...
                <div>
                  {order?.items?.map((item, i) => (
                    <div key={i} className="flex flex-col">
                      <p className="font-medium">
                        {item?.product?.name}
                        <span className="text-blue"> x {item.quantity}</span>
                      </p>
                    </div>
                  ))}
                </div>
